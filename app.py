...
def render_page(content):
    # Шаблон с тёмным фоном, оранжевыми акцентами и усиленными бордерами
    return f"""<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title>SpaceSim DEMO+</title>
  <style>
    body {{
...
def create_system_form():
    ...
    <form method='POST' action='/create_system'>
      System Name: <input type='text' name='system_name' value='Alpha Centauri'/><br/>
      ...
      Star Type: <input type='text' name='star_type' value='G-type'/><br/>
...
def create_planet_form():
    ...
    <form method='POST' action='/create_planet'>
      Planet Name: <input type='text' name='planet_name' value='Terra'/><br/>
      Planet Type: <input type='text' name='planet_type' value='Earth-like'/><br/>
...
def create_mission():
    ...
    # Предполагается, что статус по умолчанию - 'Open'
    conn = get_db_connection()
    cur = conn.cursor()
    try:
        cur.execute("""
...
########################################
# Просмотр звёздных систем (каждая система – ссылка на детали)
########################################
@app.route("/systems")
def systems():
    conn = get_db_connection()
    ...
    try:
        cur.execute(
            "SELECT system_id, system_name, coord_x, coord_y, coord_z, star_type FROM StarSystems ORDER BY system_id")
...
########################################
# Подробный просмотр системы: планеты и станции
########################################
...
def system_details(system_id):
    ...
    try:
        cur.execute(
            "SELECT system_id, system_name, coord_x, coord_y, coord_z, star_type FROM StarSystems WHERE system_id = %s",
            (system_id,))
        system = cur.fetchone()
        if not system:
            return render_page(f"<p>System with ID {system_id} not found.</p><a href='/systems'>Back</a>")
        html = f"<h2>System: {system[1]} (ID={system[0]})</h2>"
        html += f"<p>Coordinates: ({system[2]}, {system[3]}, {system[4]})<br/>Star Type: {system[5]}</p>"

        cur.execute(
            "SELECT planet_id, planet_name, planet_type, planet_size, population, is_populated FROM Planets WHERE system_id = %s ORDER BY planet_id",
            (system_id,))
...
########################################
# Тест: Расстояние между системами (func_get_distance)
########################################
...
########################################
# Тест: Поиск маршрута для корабля (func_find_path)
########################################
...
def find_path():
    ...
    cur = conn.cursor()
    try:
        cur.execute("SELECT public.func_find_path(%s::integer, %s::integer, %s::integer)", (systemA, systemB, ship_id))
...
########################################
# Апгрейд корабля (func_upgrade_ship)
########################################
...
########################################
# Генерация случайных систем и планет
########################################
...
# Изменённая логика создания корабля: единственная вставка в PlayerShips
...
def transfer_credits():
    ...
    conn.close()
    return render_page(
        f"<p>Transferred {amount} credits from player {from_id} to player {to_id}.</p><a href='/'>Back</a>")
...
def player_deal_buy(player_id):
    ...
    try:
        ...
        if not row or row[0] != player_id:
            return render_page(
                f"<p>Ship {ship_id} does not belong to player {player_id}!</p><a href='/player/{player_id}/menu'>Back</a>")
        cur.execute("""
    ...
    conn.close()
    return render_page(
        f"<p>Player {player_id} bought {quantity} units of good {good_id} for ship {ship_id} at {ppu} each.</p><a href='/player/{player_id}/menu'>Back</a>")
...
def player_deal_sell(player_id):
    ...
    conn.close()
    return render_page(
        f"<p>Player {player_id} sold {quantity} units of good {good_id} from ship {ship_id} at {ppu} each.</p><a href='/player/{player_id}/menu'>Back</a>")
...
def player_cargo(player_id):
    ...
    html += "</table><a href='/player/{player_id}/menu'>Back</a>"
    return render_page(html)
...
def player_move_ship_form(player_id):
    ...
    <form method='POST' action='/player/{player_id}/move_ship'>
      ...
      New Station ID: <input type='number' name='station_id' value='1' min="0"/><br/>
...
def player_move_ship(player_id):
    ...
    conn.close()
    return render_page(
        f"<p>Ship {ship_id} moved to system {system_id} and station {station_id}.</p><a href='/player/{player_id}/menu'>Back</a>")
...
def finish_mission_form(player_id):
    ...
    if not missions:
        return render_page(
            f"<p>No active missions for player {player_id}!</p><a href='/player/{player_id}/menu'>Back</a>")
...
def finish_mission(player_id):
    ...
    try:
        ...
        conn.close()
        return render_page(
            f"<p>Error finishing mission {mission_id}: {e}</p><a href='/player/{player_id}/menu'>Back</a>")
    cur.close()
    conn.close()
    return render_page(
        f"<p>Mission {mission_id} finished. Success = {success_bool}.</p><a href='/player/{player_id}/menu'>Back</a>")
